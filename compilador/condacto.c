...
TipoCondacto condactos_estandar[NUMCONDACTOS] = {
  {"sinCondacto", condicion, nada, nada, aNada}
  ,
  ...
  ,
/* los del spectrum */
  {"EXTERN", accion, value, nada, nada, aNada}
  ,
...
void CargarDefinicionCondacto(char *fichero)
{
	...
	f = fopen(libFile,"rt");
	fgets(linea,32768,f);
	if (linea[0]!=59) error(errGeneral,3);
	ultimo_condacto++;
	inicial = 1;
	...
	fputs(linea,fplugin); /* Copiamos resto de fichero plu en el plugin.pgl */
...
CargarCondactosUsuario ()
{
  ...
  if ((dir = opendir(libFile)) == NULL)
   ...
   {
		if (strstr(ent->d_name,".plu")!=NULL)
		{
			printf("Cargando plugin %s\n",ent->d_name);
