...
public
 ...
 property AssemblerPath: String read FAssemblerPath write FAssemblerPath;
...
begin
  ...
  FHelpBaseURL :=  IniFile.ReadString('URLS','HelpBaseURL','http://www.caad.es/superglus/doku.php/');

  FPreprocessorParameters := IniFile.ReadString('Paths','PreprocessorParameters',' -CLEAN');
...
begin
  ...
  IniFile.WriteString('Paths','LinkerPath', FullPath(FLinkerPath));
  IniFile.WriteString('Paths','AssemblerPath', FullPath(FAssemblerPath));
  IniFile.WriteString('Paths','InterpreterPath', FullPath(FInterpreterPath));
...
// Returns full path of compiler or preprocessor relative to superglus IDE, for windows it's not required so it returns the path as received
...
// Returns full path of resources relative to superglus IDE, for windows it's not required, for linux is same path than superglus IDE, for OSX is relative to path of IDE
