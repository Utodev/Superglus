...
var aProcess : TProcess;
    ...
    inQuotes: Boolean;
    i : integer;
    SuperglusLIBPATH : String;
    isEscaped : Boolean;
begin
  {$IFDEF Linux}
  SuperglusLIBPATH := GetEnvironmentVariable('Superglus_LIBPATH');
  if (SuperglusLIBPATH) = '' then SuperglusLIBPATH:=ExtractFilePath(fpReadLink('/proc/self/exe'));
  {$ENDIF}
  {$IFDEF darwin}
  SuperglusLIBPATH := GetEnvironmentVariable('Superglus_LIBPATH');
  if (SuperglusLIBPATH) = '' then SuperglusLIBPATH := ExtractFilePath(ParamStr(0)) + '../Resources/';
  {$ENDIF}
  ...
  {$IFNDEF Windows}
  aProcess.Environment.Add('Superglus_LIBPATH=' + SuperglusLIBPATH);
  {$ENDIF}
