...
begin
  ...
  begin
   CompileOutputListBox.Visible:=false;
   RunShell(Config.InterpreterPath, QuotePath(ChangeFileExt(TXP.FilePath,'.blb')));
  end;
end;
...
begin
  ...
  try
    ...
    Output := RunShell( Config.PreprocessorPath ,  Config.PreprocessorParameters + QuotePath(TXPTempFile) + '', ExtractFilePath(TXPTempFile));
    ...
    // Assembler
    if not FileExists(Config.AssemblerPath) then
    begin
      ShowMessage(S_ASSEMBLER_NOT_FOUND);
      Exit()
    end;
    CompileOutputListBox.Items.Add(S_STARTING_ASSEMBLER);
    ULATempFile :=  ChangeFileExt(TXP.FilePath,'.ula');
    DeleteFile(QuotePath(ULATempFile));
    ULXTempFile :=  ChangeFileExt(TXP.FilePath,'.ulx');
    Output := RunShell(Config.AssemblerPath,  ' -i ' + QuotePath(ULATempFile) + ' -o ' + QuotePath(ULXTempFile));
    CompileOutputListBox.Items.Text:=CompileOutputListBox.Items.Text + Output.Text;
    CompileOutputListBox.Selected[CompileOutputListBox.Items.Count-1] := true;
    if (not FileExists(ULXTempFile)) then
    begin
       ShowMessage(S_ASSEMBLER_FAIL);
       Exit();
    end;


    // Blorb packager
    if not FileExists(Config.LinkerPath) then
    begin
      ShowMessage(S_LINKER_NOT_FOUND);
      Exit()
    end;
    CompileOutputListBox.Items.Add(S_STARTING_LINKER);
    BLCTempFile:=ChangeFileExt(TXP.FilePath,'.blc');
    BLBTempFile:=ChangeFileExt(TXP.FilePath,'.blb');
    DeleteFile(QuotePath(BLBTempFile));
    Output := RunShell(Config.LinkerPath,  QuotePath(BLCTempFile) + ' ' + QuotePath(BLBTempFile),ExtractFilePath(TXP.FilePath) );
    CompileOutputListBox.Items.Text:=CompileOutputListBox.Items.Text + Output.Text;
    CompileOutputListBox.Selected[CompileOutputListBox.Items.Count-1] := true;

    if (not FileExists(BLBTempFile)) then
    begin
       ShowMessage(S_BLORB_FAIL);
       Exit();
    end;
    CompileOutputListBox.Items.Add(S_GAME_BUILT);
    CompileOutputListBox.Selected[CompileOutputListBox.Items.Count-1] := true;
...
begin
  ...
  begin
       fMain.Caption:= 'Superglus 2.0 - ' + ExtractFileName(FileName);
...
begin
 ...
 try
    ...
    for i := 0 to Context.Count - 1 do
     if pos(AnsiUpperCase(Selection),AnsiUpperCase(Context.Strings[i]))>0 then
      begin
       URL := Config.HelpBaseURL;
       if NOT (Config.HelpBaseURL[Length(Config.HelpBaseURL)] in ['/','\']) then URL := URL + '/';
       URL := URL + 'condactos:' + Context.Strings[i];
       OpenBrowser(URL);
       Found := true;
      end;
    if not Found then MHelpContents.Click();
  finally
   Context.Free;
  end;
end;
...
begin
  ...
  DeleteFile(ChangeFileExt(FileName,'.ula'));
  DeleteFile(ChangeFileExt(FileName,'.ulx'));
end;
